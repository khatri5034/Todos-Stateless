...
document.getElementById('displaySingle').addEventListener('click',async()=>{
  const id = document.getElementById('displaySingleID').value
const response =await fetch(`/todos/${id}`)
const todo= await response.json();
document .getElementById('todoDisplay').textContent=JSON.stringify(todo,null,2)
  ...
  document.getElementById('submitTodo').addEventListener('click', async() => {
    ...
    const todo = { task:name,priority,fun:isFun};
